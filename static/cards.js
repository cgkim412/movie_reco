...
class StarMovieCard extends BaseMovieCard {
	...
	_successCallback(data) {
		...
		ratingManager.applyStarRating(this.movieId, this.initScore, this.useAltColors);
	}
...
class DetailedMovieCard extends BaseMovieCard {
		...
		<div id="${this.cardId}" class="card bg-dark text-light">
			...
			<div class="card-body pb-0">
				<span class="text-center">
					...
					<h5 class="card-subtitle mb-3">(${movie.title}, ${movie.release_year})</h5>
				</span>
				...
				<p>평점: ${Math.round(movie.imdb_score * 10)}% (IMDB)<span class="mx-2">||</span> ${Math.round(movie.tmdb_score * 10)}% (TMDB)</p>
	...
	_setEventListener() {
		$(this.modalSelector).find(".read-more").on("click", function (event) {
			$(event.target).parent().parent().find('.overview-toggle').toggleClass('d-none');
		});

		$(this.modalSelector).on('hide.bs.modal', function (event) {
			$(event.target).find('.overview-toggle').toggleClass('d-none');
			$(event.target).find('.container-sim').collapse('hide');
		});

		if (this._similarItems.length > 0) {
			$(this.modalSelector).find(".show-similar-items").on("click", () => {
				const simContainerSelector = "#" + this._simContainerId;
				if ($(simContainerSelector).children().length == 0) {
					const loader = new MovieCardLoaderV(simContainerSelector, 12, 0, MoviePosterCard);
					loader.initialize(this._similarItems);
				} else {
					$(this.modalSelector).find(".show-similar-items").off("click");
				}
			});
		}
	}
