class BaseDataLoader {
  constructor(cardContainerSelector, initBatchSize, batchSize) {
    ...
    this._queue = [];
    this._isReady = true;
    this._minDelay = 300; // in millisecond
  }
...
class MovieCardLoaderV extends BaseDataLoader {
  constructor(cardContainerSelector, initBatchSize = 10, batchSize = 5, cardClass) {
  ...
  _setEventListener() {
    $(window).scroll(() => {
      if ($(document).height() <= $(window).scrollTop() + $(window).height() + this._scrollOffset) {
        this._eventHandler();
      }
    })
  }
...
class RatedMovieCardLoader extends MovieCardLoaderV {
  constructor(cardContainerSelector, initBatchSize = 10, batchSize = 5, cardClass = StarMovieCard) {
    super(cardContainerSelector, initBatchSize, batchSize);
    this._cardClass = cardClass;
    this._scrollOffset = 400;
  }
...
class MovieCardLoaderH extends BaseDataLoader {
  constructor(cardContainerSelector, initBatchSize = 5, batchSize = 3, cardClass) {
    super(cardContainerSelector, initBatchSize, batchSize);
    this._cardClass = cardClass;
    this._minDelay = 450;
  ...
  _setEventListener() {
    $(this.cardContainerSelector).siblings(".arrow-right").on("click", () => {
      this._eventHandler();
    });
  }

  _unbindEventListener() {
    $(this.cardContainerSelector).siblings(".arrow-right").off("click");
  }
...
class CardContainerLoader extends MovieCardLoaderV {
  constructor(parentSelector, initBatchSize = 3, batchSize = 1, containerClass) {
  ...
  _deploySingleBatch(size=this.batchSize) {
    for (let i = 0; (i < size && this._queue.length > 0); i++) {
      ...
      const container = new this.containerClass(this._indexCount, containerData.label, containerData.items.length);
      ...
      const loader = new MovieCardLoaderH(container.selector, 8, 4, SimpleMovieCard);
      // currently item scores are not used
      // const movieIds = [];
      // containerData.items.forEach((item) => movieIds.push(item.id));
...
const eventManager = {
  ...
  activateMovieDetailModals: function() {
    ...
    $(this.mainContainerSelector).on("click", ".card-container .card-clickable", function() {
      const detailCard = new DetailedMovieCard($(this).parent().attr('movie-id'));
      detailCard.showAsModal();      
    });

    $(this.modalContainerSelector).on("click", ".modal .container-sim .card-clickable", (event) => {
      $(this.modalContainerSelector).find(".modal, show").modal("hide");
      const detailCard = new DetailedMovieCard($(event.currentTarget).parent().attr('movie-id'));
      detailCard.showAsModal();
    });
